...
- `get_rates_summary` (`rates.py`):
  ...
  Если в реплике есть сумма рублей, а не бат,, то нужен `think=true`
...
- `get_rshb_report` (`rates.py rshb`):
  ...
  «рсхб / курсы рсхб» без чисел в текущей реплике => `arguments: {{}}` (дефолт: 30000, комиссия 250) c `think=false`
